...
//	let term = '/%/'+req.params.user + '/%/'
        let sqlquery =	`SELECT *
                        FROM users
			WHERE user_id LIKE ? `;

		//	JOIN topic t
		//	ON t.topic_id =u.user_id
    ...
    app.get('/search-result', function (req, res) {
        //searching in the database
	      ...
	      let sqlquery = `SELECT *
			      ...
			      WHERE  post_title LIKE ? OR post_content LIKE ? OR topic_title LIKE ? 
			      ORDER BY post_date DESC`;

        // Run the query
        db.query(sqlquery,[term,term,term], (err, result) => {
          if (err) {
             res.redirect('./');
          }
